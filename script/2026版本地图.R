...
ebird_taxonomy <- read.csv("data/ebird_taxonomy.csv")
df <- read.csv("data/MyEBirdData.csv")
...
df.world.record <- world_map %>%
  left_join(df.world.record, by=c("iso_a2"="Country_code"))

q <- ggplot(df.world.record) +
  # 绘制地图
  geom_sf(aes(fill = n_species), color = "white", size = 0.05) +
